...
void mocRenderObjects(void* moc, objectRenderData* data, int nObject, int* resultList, float* PxVMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask)
{
	float MVPMatrix[16];
	for (int i = 0; i < nObject; i++)
	{
		mulMatrix4x4(PxVMatrix, data[i].modelToClipMatrix, MVPMatrix);
		resultList[i] = (int)(((MaskedOcclusionCulling*)moc)->RenderTriangles(data[i].inVtx, data[i].inTris, data[i].nTris, MVPMatrix, bfWinding, clipPlaneMask, VertexLayout(16, 4, 12)));
	}
}
...
void mocTestObjects(void* moc, objectRenderData* data, int nObject, int* resultList, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask)
{
	...
	for (int i = 0; i < nObject; i++) 
	{
		resultList[i] = (int)(((MaskedOcclusionCulling*)moc)->TestTriangles(data[i].inVtx, data[i].inTris, data[i].nTris, data[i].modelToClipMatrix, bfWinding, clipPlaneMask, VertexLayout(16, 4, 12)));
	}
}
